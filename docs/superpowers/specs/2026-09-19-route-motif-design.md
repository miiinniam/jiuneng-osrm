...
- 画布 `viewBox="0 0 1160 168"`，`preserveAspectRatio="xMidYMid meet"`（**保持形状，禁止拉伸**）。
- **容器必须锁定比例**：`aspect-ratio: 1160 / 168`。这是防变形的关键——若只固定高度 168px 而宽度随视口变化，SVG 会被纵向拉伸，路线形状失真。用 `aspect-ratio` 让宽高同比缩放。
...
2. **端点标签与圆点脱节**：右侧标签浮在端点上方约 120px，指向不明。→ 标签位置需锚定到端点坐标，而非固定在容器顶部。
